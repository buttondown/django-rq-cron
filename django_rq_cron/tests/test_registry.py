...
def test_register_cron_with_parameters():
    ...
    assert (
        REGISTERED_CRON_JOBS["test_cron_with_params"].function == test_cron_with_params
    )
    assert (
        REGISTERED_CRON_JOBS["test_cron_with_params"].cadence == CronJob.Cadence.DAILY
    )
    assert (
        REGISTERED_CRON_JOBS["test_cron_with_params"].description
        == "Test cron with parameters"
    )
